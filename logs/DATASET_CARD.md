...
That key name is inaccurate; it is retained for schema compatibility with the test suite.
...
This is not a converged steady state. Use the phrase "Vmem after 5 s equilibration" in any
...
before plateauing. The non-monotonic shape indicates two superposed processes rather than a single
relaxation:
...
The PRD sec 4.1.4 convergence criterion is therefore unsatisfiable as written for this parameter
...
5. `Nav` drives passive `Dm_Na`, not the voltage-gated channel entry. Raising the
   Nav1p3 channel's `max Dm` from 0 to 1e-15 moved Vmem by 0.08 mV, because a voltage-gated sodium
   channel is shut at the resting potential. This is correct physics, not a bug.
...
feature said 4x, producing an input that disagreed with its own target.
...
## OOD difficulty is highly heterogeneous
...
channel range: Nav 0 to 49.8, Cl 0 to 15.0, Ca 0 to 9.98). The disparity is physics, not a generation
...
Roughly one third of spatial perturbations constitute a genuine generalization test; the rest sit close
...
same simulator takes approximately 42 s for tissues of 40-232 cells.

Use 117.2 s when reasoning about campaign wall-clock time at 12 workers. Use the
serial figure when reasoning about per-simulation latency, which is the quantity any
...
The MLP's v2 error (3.008 mV) represents a ceiling, as the architecture cannot propagate
information from neighbors through gap junctions. The MPNN's 2.81x advantage confirms the graph
hypothesis, which was undetectable on v1 data where the margin was 0.03 mV.

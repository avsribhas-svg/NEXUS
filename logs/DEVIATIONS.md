...
the MPNN beats the MLP by 2.81× (1.070 vs 3.008 mV test_id MAE), confirming the graph hypothesis.

**The v1→v2 contrast is the project's central result.** Effect size went from 0.03 mV to 1.94 mV
-- a 94× increase from one sampling change.
...
BETSE median 41.32 s, model CPU 22.27 ms (1,856×), model CUDA 7.40 ms (5,582×). See §12.2 of
the research report.

### D16 [N] -- ~~Ablations partially redundant~~ **Resolved on v2 data**
*PRD §7.4 lists five ablations.*
...
**v1:** Converged, seed 42: MPNN `test_id` MAE 0.761 vs MLP 0.812 -- a 6.3% margin for 25× the
parameters. Seed 137: 0.780 vs 0.780, a dead tie. The hypothesis was untested because the data
could not express it (D8).

**v2:** MPNN 1.070 vs MLP 3.008 -- **2.81× advantage**. The MLP early-stops at epoch 35 (ceiling
